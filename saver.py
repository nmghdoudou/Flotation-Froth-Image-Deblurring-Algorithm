...
class Saver():
  ...
  # save result images
  def write_img(self, ep, it,model):
    if (ep + 1) % self.img_save_freq == 0:
        assembled_images = model.assemble_outputs()
        blur_B = model.outputs_real_B_encode()
        deblur_B = model.outputs_fake_I_encoded()

        image_name = 'gen_' + str(ep) + '_' + str(it) + '.jpg'
        image_name_blur_B = 'gen_blur_' + str(ep) + '_' + str(it) + '.jpg'
        image_name_deblur_B = 'gen_deblur_' + str(ep) + '_' + str(it) + '.jpg'

        img_filename = '%s/%s' % (self.image_dir, image_name)
        img_filename_blur_B = '%s/%s' % (self.image_dir, image_name_blur_B)
        img_filename_deblur_B = '%s/%s' % (self.image_dir, image_name_deblur_B)

        #img_filename = '%s/gen_%05d.jpg' % (self.image_dir, ep)
        torchvision.utils.save_image(assembled_images / 2 + 0.5, img_filename, nrow=1)
        torchvision.utils.save_image(blur_B/ 2 + 0.5, img_filename_blur_B, nrow=1)
        torchvision.utils.save_image(deblur_B/ 2 + 0.5, img_filename_deblur_B, nrow=1)
    elif ep == -1:
        assembled_images = model.assemble_outputs()
