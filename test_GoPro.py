...
def main():
    ...
    orig_dir = os.path.join(opts.orig_dir)
    ...
    for (deblur_img_name, orig_img_name,blur_img_name) in zip(deblur_list, orig_list,blur_list):
        ...
        deblur_img_path = os.path.join(result_dir, deblur_img_name)
        orig_img_path = os.path.join(orig_dir, orig_img_name)
        blur_img_path = os.path.join(blur_dir, blur_img_name)

        """
        deblur_img = imread(deblur_img_name)
        orig_img_temp = imread(orig_img_name)
        orig_img = resize(orig_img_temp,256)
        """
        deblur_img = cv2.imread(deblur_img_path)

        orig_img_temp = cv2.imread(orig_img_path)
        orig_img = cv2.resize(orig_img_temp,(256,256))
        file_name = '/home/projects/src/outputs/clear_orign_resize_image/' + orig_img_name
        cv2.imwrite(file_name,orig_img)

        blur_img_temp = cv2.imread(blur_img_path)
        blur_img = cv2.resize(blur_img_temp, (256, 256))
        file_name = '/home/projects/src/outputs/blur_orign_resize_image/' + blur_img_name
        ...
        try:
            ...
            print("image={},psnr={},ssim={}".format(orig_img_name,psnr[-1],ssim[-1]))
