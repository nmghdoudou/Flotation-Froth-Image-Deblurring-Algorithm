...
class UID(nn.Module):
  ...
  def forward(self):
    ...
    #self.real_I_random = real_I.clone()
    #self.real_B_encoded = real_B
    #self.real_B_random = real_B.clone()
    ...
    if self.concat:
      ...
      self.mu_i, self.logvar_i = self.enc_a.forward(self.real_I_encoded)
      std_i = self.logvar_i.mul(0.5).exp_()
      eps_i = self.get_z_random(std_i.size(0), std_i.size(1), 'gauss')
      self.z_attr_i = eps_i.mul(std_i).add_(self.mu_i)
    else:
    ...
    input_attr_forI = torch.cat((self.z_attr_b, self.z_attr_i, self.z_random), 0)
    ...
    if self.concat:
      ...
      """
      self.mu_recon_i, self.logvar_recon_i = self.enc_a.forward(self.fake_I_encoded)
      std_i = self.logvar_recon_i.mul(0.5).exp_()
      eps_i = self.get_z_random(std_i.size(0), std_i.size(1), 'gauss')
      self.z_attr_recon_i = eps_i.mul(std_i).add_(self.mu_recon_i)
      """
    else:
    ...
    # for display
    self.image_display = torch.cat((self.real_I_encoded[0:1].detach().cpu(), self.fake_B_encoded[0:1].detach().cpu(), \
                                    self.fake_B_random[0:1].detach().cpu(), self.fake_II_encoded[0:1].detach().cpu(),
                                    self.fake_I_recon[0:1].detach().cpu(), \
                                    self.real_B_encoded[0:1].detach().cpu(), self.fake_I_encoded[0:1].detach().cpu(), \
                                    self.fake_I_random[0:1].detach().cpu(), self.fake_BB_encoded[0:1].detach().cpu(),
  ...
  def backward_EG(self):
    ...
    fake = self.tensor2im(self.fake_I_encoded)
    real = self.tensor2im(self.real_B_encoded)
    self.psnr = PSNR(fake,real)
    self.ssim = SSIM(fake,real,multichannel=True)
  ...
  def backward_G_alone(self):
    ...
    loss_G2 = loss_G2.detach_().requires_grad_(True)
    loss_G2.backward()
  ...
  def outputs_real_B_encode(self):
    images_b = self.normalize_image(self.real_B_encoded).detach()
    real_B_encoded = images_b[0:1, ::]
    return real_B_encoded

  def outputs_fake_I_encoded(self):
    images_a1 = self.normalize_image(self.fake_I_encoded).detach()
    fake_I_encoded = images_a1[0:1, ::]
    return fake_I_encoded
